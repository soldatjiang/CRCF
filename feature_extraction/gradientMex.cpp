...
// compute hog13 features
void hog13( float *M, float *O, float *H, int h, int w, int binSize,
  int nOrients, int softBin, float clip )
{
    float *N, *R; const int hb=h/binSize, wb=w/binSize, nb=hb*wb, nbo=nb*nOrients;
    // compute unnormalized gradient histograms
    R = (float*) wrCalloc(wb*hb*nOrients,sizeof(float));
    gradHist( M, O, R, h, w, binSize, nOrients, softBin, false );
    // compute block normalization values
    N = hogNormMatrix( R, nOrients, hb, wb, binSize );
    // sum over all normalizations
    hogChannels( H+nbo*0, R, N, hb, wb, nOrients, clip, 1 );
    // sum over all orientations
    hogChannels( H+nbo*1, R, N, hb, wb, nOrients, clip, 2 );
    wrFree(N); wrFree(R);
}
...
void mGradHist( int nl, mxArray *pl[], int nr, const mxArray *pr[] ) {
  ...
  nChns = useHog== 0 ? nOrients : (useHog==1 ? nOrients*4 : (useHog==2 ? nOrients*3+5 : nOrients+4));
  ...
  if( useHog==0 ) {
    ...
    hog13( M, O, H, h, w, binSize, nOrients, softBin, clipHog );
  } else {
     mexErrMsgTxt("No such feature option.");
  }
}
